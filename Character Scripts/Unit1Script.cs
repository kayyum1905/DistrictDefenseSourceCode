...
public class Unit1Script : UnitScript
{
	protected override void Start ()
	{
		...
		// initial health set in editor
		health = initialHealth;
		// get animator component
		anim = GetComponent<Animator>();
		// there are time wen this is for use
		spriteRenderer = GetComponent<SpriteRenderer>();
		//while dragging sprite must not flow behing any object on the field
		spriteRenderer.sortingOrder = 8;

		audioSource = GetComponent<AudioSource>();

		//others setting are done via the parent class
		base.Start();
	...
	{
		// while dragging if the collided unit is gone (which does not cause for trigger exit
		// then the unit will be set for can assemble
		...
		// in case unit is not positoned by dragging but by pressing the grid
		...
		if(!playWithTouch)
		{
			...
			if(Input.GetMouseButton(0) && canAssemble && !positioned)  // with mouse
			{
				...
				if(pay == true)
					PositioningSetups(); //to avoid writing the same codes else if playing with touch & better arranging
			}
		}
	...
	{
		// the ordering of lines below are critic
